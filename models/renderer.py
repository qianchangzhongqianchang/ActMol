...
class MVRenderer(nn.Module):
    ...
    def __init__(self, nb_views, image_size=224, pc_rendering=True, object_color="white", background_color="white", faces_per_pixel=1, points_radius=0.006,  points_per_pixel=1, light_direction="random", cull_backfaces=False):
        ...
        # self.device = torch.device(device)  # <-- 添加这一行
    ...
    def render_meshes(self, meshes, color, azim, elev, dist, lights, background_color=(1.0, 1.0, 1.0), ):
        ...
        new_meshes.textures = Textures(
            verts_rgb=color.to(self.device)*torch.ones((c_batch_size, max_vert, 3)).to(self.device))
        ...
        # camera = OpenGLPerspectiveCameras(device=self.device, R=R[None, 0, ...],
        #                                   T=T[None, 0, ...])
    ...
    def render_points(self, points, color, azim, elev, dist, background_color=(0.0, 0.0, 0.0), ):
        ...
        point_cloud = Pointclouds(points=points.to(torch.float), features=color *
                                  torch.ones_like(points, dtype=torch.float))
        # # 将 .cuda() 替换为 .to(self.device)
        # point_cloud = Pointclouds(points=points.to(torch.float), features=color * torch.ones_like(points,
        ...
        # # 将 device="cuda:..." 替换为 device=self.device
        # cameras = OpenGLOrthographicCameras(device=self.device, R=R, T=T, znear=0.01)
    ...
    def forward(self, meshes, points, azim, elev, dist, color=None):
        ...
        background_color = torch_color(
            self.background_color, max_lightness=True, ).to(points.device)
        # # 将 .cuda() 替换为 .to(self.device)
        # background_color = torch_color(self.background_color, max_lightness=True, ).to(self.device)
